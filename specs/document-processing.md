...
- **Manual reprocessing (`FR-PROC-005`):** a user can trigger reprocessing of a `failed` document (e.g., after re-uploading a fixed version, or if a transient cause is suspected). This creates a fresh processing run from `queued`, does not silently reuse stale partial state (any prior `document_chunks` rows for that document are deleted before reprocessing begins, to avoid duplicate/orphaned chunks).
