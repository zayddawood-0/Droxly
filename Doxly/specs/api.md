...
- **Response 200:** paginated `{ document_id, file_name, snippet: string (match highlighted), relevance_score: float, matched_page: int|null }`, ranked by the hybrid keyword (Postgres `tsvector`) + vector similarity score defined in `rag.md` §Hybrid Search (`FR-SEARCH-003`). Always scoped to the caller's own, non-deleted documents (`NFR-SEC-001`).
