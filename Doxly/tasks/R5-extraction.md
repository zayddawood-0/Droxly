...
## Independent Audit (2026-08-26) and Remediation

An independent, read-only audit of the committed implementation (then `a8a174f`) found the requirement/API/observability/security implementation genuinely correct, with two MEDIUM findings — both since fixed and committed (`6601e3c`):

1. **`AnthropicLLMProvider.generate_structured` discarded real, already-metered usage** whenever the structured-output *gate* failed after a successful HTTP call (no `tool_use` block, or a payload failing Pydantic validation) — the resulting `ai_requests` row would log `None`/`"n/a"` for a call the provider actually metered. **Fixed:** `StructuredOutputError` now carries optional `input_tokens`/`output_tokens`/`model` (defaulting to `None`, never fabricated); both failure branches in `generate_structured` attach the already-fetched usage; `extraction_agent_node`'s except-branch propagates it into `ExtractionState` so the existing `_log_ai_request` code needed no changes at all.
2. **`extraction_agent_node`/`document_classifier_node`'s system prompts didn't fully match `ai.md` §4 / `NFR-SEC-007`'s prompt-injection defense pattern** already established by `document_qa.py` (R4) — missing the explicit "disregard instruction-like text inside document content" framing, and (for the classifier) missing `<document_context>` delimiters entirely. **Fixed:** both nodes now carry the identical framing/delimiter pattern `document_qa.py` uses.

Both fixes are additive/backward-compatible (no existing caller of `generate_structured` or the two node functions needed to change beyond the two other pre-existing scaffold callers already touched by ADR-027) and required no new ADR — they close gaps in already-decided directions (ADR-027's usage-preservation intent; `ai.md` §4's already-documented policy), not new architectural decisions. 7 new regression tests were added (`test_llm_provider.py` ×4 — mocking `AnthropicLLMProvider._call` directly, since no wire-level HTTP mock convention exists anywhere in this codebase; `test_extraction_processing_service.py` ×1; `test_graph_extraction.py` ×2, asserting the actual constructed system prompt/message content, not just successful node execution).

A LOW finding (a handful of negative-path tests in `test_extractions_api.py` asserting only status codes, not the full error envelope) was left open at the time as explicitly optional/non-blocking.

## Reconciliation Pass (2026-08-27)

Re-read `requirements.md` §1.8, `api.md` §6, `database.md` §3.11, `testing.md` (§1.2's P0/P1 test-coverage mandate, §4.4's Extraction-graph test assertions, §6's requirement-to-test naming convention), `architecture.md`, `decisions.md`, and `tasks/remediation-plan.md` §8 against the current, committed code (`6601e3c`) to determine what R5 scope, if any, remained unfinished.

**Finding:** every `FR-EXT-00{1,2,3,4}` and `NFR-OBS-001` acceptance criterion is implemented and test-covered; `remediation-plan.md` §8's four required test categories (graph-level, API, schema-validation-rejection, `ai_requests`-row assertion) are all present. The three items previously listed under Known Limitations (stale-extraction recovery, no failure-reason field, the inherited `NFR-AVAIL-002` retry-count ambiguity) are not unfinished R5 work — each is either explicitly outside `api.md` §6's documented contract (inventing either would be scope creep, not closing a gap) or a pre-existing, R3-originated ambiguity awaiting a product decision, not a coding task. `testing.md` §6.1's `test_fr_ext_003_...`-style naming convention is aspirational project-wide (no test in the entire codebase — R1 through R5 — follows it), so it is not an R5-specific gap either.

The one genuine, narrowly-scoped remaining item was the previously-deferred LOW finding: **7 tests in `test_extractions_api.py` asserted only an HTTP status code where the project's error envelope (`{error: {code, message, request_id}}`) was actually returned and should have been checked** (`test_list_templates_requires_auth`, `test_create_extraction_rejects_generic_as_a_template_key`, `test_create_extraction_404s_for_another_users_document`, `test_create_extraction_requires_auth`, `test_get_extraction_404s_for_missing_or_other_users_extraction` [both sub-cases], `test_list_document_extractions_404s_for_another_users_document`, `test_patch_extraction_404s_for_another_users_extraction`). **Fixed** — each now asserts `response.json()["error"]["code"]` against the exact documented code (`unauthorized`, `validation_error`, `not_found`), closing the gap `security-testing`'s error-envelope-consistency expectation calls for. No new test functions were added (all 19 pre-existing `test_extractions_api.py` tests remain, now with stronger assertions); no source/behavior changed.

**Conclusion: R5 (Extraction) has no remaining unfinished scope.** All P0/P1 requirements are implemented, tested, and independently audited; both audit findings are fixed; the one deferred low-severity test-quality item is now closed. Nothing further is pending short of a product decision on the two explicitly-out-of-contract limitations (stale-extraction recovery, failure-reason surfacing) or the inherited retry-count wording question — neither of which api.md currently asks for.
